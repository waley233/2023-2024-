...
def set_iptables_rule(chain, protocol, src_ip, dst_ip, src_port, dst_port, action="DROP"):
    ...
    try:
        subprocess.run(rule, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        return True  # 返回 True 表示规则设置成功
    except subprocess.CalledProcessError as e:
        print(f"Error: {e}")
        return False  # 返回 False 表示规则设置失败
...
def on_submit():
    ...
    success = False
    for chain in ['INPUT', 'OUTPUT', 'FORWARD']:
        success = set_iptables_rule(chain, protocol, src_ip, dst_ip, src_port, dst_port, action)
        if not success:
            break

    if success:
        messagebox.showinfo("Success", "The rules have been set.")
        print("The rules have been set.")  # 在命令行中显示信息
    else:
        messagebox.showerror("Error", "Failed to set rules.")
        print("Failed to set rules.")  # 在命令行中显示信息
...
#sudo python3 firewall6.py --protocol tcp --src_ip 192.168.23.129 --dst_ip 192.168.23.128 --src_port 23 --dst_port 23 --action DROP
#sudo iptables -F
